    ...
    <div style={{ background: 'var(--cream)' }}>
      <div style={{ padding: '16px 40px 0', background: 'var(--cream)' }}>
        <div style={{ position: 'relative', width: '100%', height: '500px', overflow: 'hidden', borderRadius: '16px', border: '1px solid var(--border)' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(10,10,10,0.88) 0%, rgba(10,10,10,0.55) 45%, rgba(10,10,10,0.15) 100%)' }} />
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 75% 50%, rgba(201,116,138,0.08) 0%, transparent 60%)', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', inset: 0, zIndex: 2, padding: '0 56px', display: 'flex', alignItems: 'center' }}>
            <div style={{ maxWidth: '520px', opacity: animating ? 0 : 1, transform: animating ? 'translateY(12px)' : 'translateY(0)', transition: 'opacity 0.5s ease 0.1s, transform 0.5s ease 0.1s' }}>
              ...
              <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.65)', lineHeight: 1.8, marginBottom: '32px', fontWeight: 300, maxWidth: '400px' }}>{s.sub}</p>
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                <a href="#machines" style={{ background: 'var(--white)', color: 'var(--ink)', padding: '13px 28px', fontSize: '11px', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', textDecoration: 'none', display: 'inline-block', transition: 'background 0.2s, color 0.2s' }}
                ...
                <a href="#machines" style={{ fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', textDecoration: 'none', fontWeight: 500, transition: 'color 0.2s' }}
                  onMouseEnter={e => e.target.style.color = '#fff'}
                  onMouseLeave={e => e.target.style.color = 'rgba(255,255,255,0.5)'}>View all →</a>
              </div>
            </div>
          </div>
          <button onClick={() => goSlide((currentSlide - 1 + HERO_SLIDES.length) % HERO_SLIDES.length)}
            style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.7)', width: '40px', height: '40px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', zIndex: 3, borderRadius: '4px', transition: 'all 0.2s' }}
            ...
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(0,0,0,0.5)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.color = 'rgba(255,255,255,0.7)' }}>←</button>
          <button onClick={() => goSlide((currentSlide + 1) % HERO_SLIDES.length)}
            style={{ position: 'absolute', right: '16px', top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.7)', width: '40px', height: '40px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', zIndex: 3, borderRadius: '4px', transition: 'all 0.2s' }}
            onMouseEnter={e => { e.currentTarget.style.background = 'var(--rose)'; e.currentTarget.style.borderColor = 'var(--rose)'; e.currentTarget.style.color = '#fff' }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(0,0,0,0.5)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.color = 'rgba(255,255,255,0.7)' }}>→</button>
      ...
      <div style={{ background: 'var(--ink)', margin: '0 40px', borderRadius: '0 0 12px 12px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderLeft: '1px solid var(--border-dark)' }}>
          {[
            ...
            <div key={i} style={{ borderRight: '1px solid var(--border-dark)', padding: '24px 24px', display: 'flex', alignItems: 'center', gap: '14px' }}>
              <div style={{ width: '36px', height: '36px', flexShrink: 0, border: '1px solid var(--border-dark)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--rose)' }}>{f.icon}</div>
              <div>
      ...
      <div id="machines" style={{ maxWidth: '1400px', margin: '0 auto', padding: '72px 40px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '48px', paddingBottom: '16px', borderBottom: '1px solid var(--border)' }}>
          <div>
            ...
            <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(32px,3.5vw,48px)', letterSpacing: '0.02em', color: 'var(--ink)', lineHeight: 0.95 }}>ALL MACHINES</h2>
          </div>
            ...
            return (
              <div key={p.id} style={{ background: 'var(--white)', display: 'flex', flexDirection: 'column', transition: 'background 0.2s' }}
                ...
                <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--muted)', lineHeight: 1.6, marginBottom: '16px', fontWeight: 300 }}>{p.description || p.desc}</div>
                  {specs.length > 0 && (
                  ...
                  <div style={{ marginTop: 'auto' }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', marginBottom: '16px' }}>
                      <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '26px', letterSpacing: '0.02em', color: 'var(--ink)' }}>Rs.{p.price.toLocaleString()}</span>
      ...
      <div style={{ position: 'relative', margin: '0 40px 40px', height: '320px', overflow: 'hidden', borderRadius: '12px', border: '1px solid var(--border)' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(10,10,10,0.9) 0%, rgba(10,10,10,0.5) 50%, transparent 100%)', display: 'flex', alignItems: 'center', padding: '0 56px' }}>
          <div>
            ...
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(36px,4vw,56px)', lineHeight: 0.9, color: '#ffffff', marginBottom: '20px' }}>TALK TO OUR<br />EXPERTS</div>
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', lineHeight: 1.7, maxWidth: '280px', marginBottom: '28px', fontWeight: 300 }}>Get personalized recommendations for your printing business.</p>
            <Link href="/contact" style={{ background: 'var(--white)', color: 'var(--ink)', padding: '13px 28px', fontSize: '11px', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', textDecoration: 'none', display: 'inline-block', transition: 'background 0.2s' }}
              onMouseEnter={e => { e.target.style.background = 'var(--rose)'; e.target.style.color = '#fff' }}
